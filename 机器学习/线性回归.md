...
这个损失函数是关于$\ \beta$的 的二次型函数。
将$S SSR$对 对向量$\ \beta$求 求导，可得最小化问题的一阶条件
...
前提是$( (\mathbf{X}'\mathbf{X})^{-1}$可 可逆，则X满列秩。
换言之，不存在某个列向量可被其他列向量线性表出的情形
如果数据矩阵 X 不满列秩，则称存在 严格多重共线性。

OLS 一般不适用于“高维数据”(high-dimensional data)。
由于高维数据的变量个数大于样本容量  即$p p<n$。 。
故$r rank(X) <n< p$( (矩阵X的秩小于或等于其行数n)，故数据矩阵X不满列秩。此时$( (\mathbf{X}'\mathbf{X})^{-1}$不 不存在，故 OLS 没有唯一解。
...
$\ \hat{\mathbf{y}}'\mathbf{e} = (\mathbf{X}\hat{\boldsymbol{\beta}})'\mathbf{e} = \hat{\boldsymbol{\beta}}'\underbrace{\mathbf{X}'\mathbf{e}}_{= 0} = \hat{\boldsymbol{\beta}}' \cdot \mathbf{0} = 0$可 可得^y也与残差正交。
...
$P P_X$为 为**投影矩阵**。其作用是将y (或任何n维列向量)投影到X的列空间col( X)
...
$M M_X$为 为**消灭矩阵**。其作用是将y向X的列空间col( X) 投影，然后得到此投影的残差。
...
$\ \sum_{i=1}^{n} (y_i - \bar{y})^2$( (平方和分解公式$S SST=SSR+SSE$) )
**当回归方程有常数项。**

证明：将$y y_i - \bar{y}$分 分解成$y y_i - \hat{y}_i+\hat{y}_i-\bar{y}=e_i+\hat{y}_i-\bar{y}$得 得
...
根据平方和分解公式，$y y_i$的 的离差平方$\ \sum_{i = 1}^{n}(y_i-\bar{y})^2$可 可分解成不可解释部分$\ \sum_{i = 1}^{n}e_i^2$和 和可解释部分$\ \sum_{i = 1}^{n}(\hat{y}_i-\bar{y})^2$。 。
...
生成模拟数据，并画散点图与函数$f f(x)=\sin(2\pi x)$
...
$$
由上式可知，偏差度量的是在大量重复抽样过程中，$\ \hat{f}(X)$对 对于$f f(X)$的 的平均偏差程度。

方差(Variance)则衡量在大量重复抽样过程中，估计量$\ \hat{f}(X)$本 本身围绕着其期望值$E E \hat{f}(X)$
的波动幅度，其定义为
（对于估计值的方差定义式）

$$\text{Var}(\hat{f}(\mathbf{x})) \equiv \mathbb{E}\left[ \hat{f}(\mathbf{x}) - \mathbb{E}\hat{f}(\mathbf{x}) \right]^2
$$
给定X，估计量$\ \hat{f}(X)$的 的均方误差可作如下分解：
...
$\ \hat{f}(X) - E\hat{f}(X)$是 是常数
$E E(\varepsilon)^2 = E(\varepsilon^2) -E(\varepsilon)^2 = E(\varepsilon^2)$               ($E E(\varepsilon) = 0$) )
...
因此，估计量$\ \hat{f}(X)$的 的均方误差可分解为偏差平方$( (Bias^2)$、 、方差与扰动项方差$V Var(\varepsilon)$之 之和。
...
另一方面，扰动项方差$V Var(\varepsilon)$则 则“不可降低”(irreducible)，即使知道真实函数 f(x) ，但$V Var(\varepsilon)$也 也依然存在。
本质上，扰动项的方差$V Var(\varepsilon$) )  决定了预测问题的困难程度。
如果扰动项方差$V Var(\varepsilon)$很 很大，即使你所学得的函数$\ \hat{f}(X)$已 已经很接近于真实函数$f f(X)$   ，预测准确度依然可能不高；比如对金融市场的预测。
另一方面，如果扰动项方差很小，则只要$\ \hat{f}(X)$足 足够接近$f f(X)$   ，预测准确度就可以很高；比如人脸识别。
